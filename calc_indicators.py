...
def compute_ichimoku(data, prefix="D_", weekly=False):
    ...
    ichimoku[f"{prefix}Senkou_span_A_future"] = ichimoku[
        f"{prefix}Senkou_span_A"
    ].shift(-26)
    ichimoku[f"{prefix}Senkou_span_B_future"] = ichimoku[
        f"{prefix}Senkou_span_B"
    ].shift(-26)

    return ichimoku


def add_weekly_senkou_b_slope_features(ichimoku_df: pd.DataFrame) -> pd.DataFrame:
    """
    Add smoothed Senkou Span B slope features for WEEKLY Ichimoku.
    Assumes columns already exist:
    - W_Senkou_span_B
    """
...
def compute_heikin_ashi(data, prefix="D_", weekly=False):
    ...
    ha[f"{prefix}HA_High"] = pd.concat(
        [data[f"{prefix}High"], ha[f"{prefix}HA_Open"], ha[f"{prefix}HA_Close"]], axis=1
    ).max(axis=1)

    ha[f"{prefix}HA_Low"] = pd.concat(
        [data[f"{prefix}Low"], ha[f"{prefix}HA_Open"], ha[f"{prefix}HA_Close"]], axis=1
    ).min(axis=1)
...
def compute_bollinger_bands(data, period=20, std_dev=2, prefix="D_"):
    ...
    bb[f"{prefix}BB_Width_{period}"] = (
        bb[f"{prefix}BB_Upper_{period}"] - bb[f"{prefix}BB_Lower_{period}"]
    )
...
def compute_ATR(data, periods=[14], prefix="D_", weekly=False, wilder=True):
    ...
    for p in periods:
        raw_atr = (
            ...
            if wilder
            else tr.rolling(p, min_periods=p).mean()
        )
