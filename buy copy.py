...
from trend_channel_buy_helper import compute_latest_trend_channel
...
def buy_check(open_trades, data, i, cash, buy_markers, equity, trades):
    ...
    # Compare width to prior days to detect expansion
    bb_width_prev = data['D_BB_Upper_20'].iloc[i-1] - data['D_BB_Lower_20'].iloc[i-1]
    bb_expanding = bb_width > bb_width_prev and bb_width > data['D_BB_Middle_20'].iloc[i] * 0.02  # threshold: >2% of mid-price
    ...
    # === PRELIMINARY buy conditions (without channel) ===
    pre_buy_signal = (
    ...
    if pre_buy_signal:
        # ✅ Now run the trend channel check only if all the above pass
        #data, channel_breakout = compute_latest_trend_channel(data, i, debug=True)
        if True:#channel_breakout:  # Final confirmation
            ...
            if risk_per_unit > 0:
                ...
                if cash >= cost:
                    print(f"✅ BUY [{current_date}] @ {close:.2f} (Channel Breakout Confirmed)")
